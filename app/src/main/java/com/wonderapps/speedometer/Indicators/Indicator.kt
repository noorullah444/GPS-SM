...
import com.wonderapps.speedometer.Speedometer
...
abstract class Indicator<I : Indicator<I>> protected constructor(context: Context) {
    ...
    companion object {
        ...
        fun createIndicator(context: Context, indicator: Indicators): Indicator<*> {
            when (indicator) {
                Indicators.NoIndicator -> return NoIndicator(context)
                Indicators.NormalIndicator -> return NormalIndicator(context)
                Indicators.NormalSmallIndicator -> return NormalSmallIndicator(context)
                Indicators.TriangleIndicator -> return TriangleIndicator(context)
                Indicators.SpindleIndicator -> return SpindleIndicator(context)
                Indicators.LineIndicator -> return LineIndicator(context, LineIndicator.LINE)
                Indicators.HalfLineIndicator -> return LineIndicator(context, LineIndicator.HALF_LINE)
                Indicators.QuarterLineIndicator -> return LineIndicator(context, LineIndicator.QUARTER_LINE)
                Indicators.KiteIndicator -> return KiteIndicator(context)
                Indicators.NeedleIndicator -> return NeedleIndicator(context)
